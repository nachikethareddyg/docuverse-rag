...
* The SOAP API does not support S3 Versioning. SOAP support over HTTP is deprecated, but
  it is still available over HTTPS. New Amazon S3 features are not supported for SOAP.
...
You enable and suspend versioning at the bucket level. After you version-enable a
bucket, it can never return to an unversioned state. But you can *suspend* versioning on that bucket.

The versioning state applies to all (never some) of the objects in that bucket. When
...
enabled will thereafter *always* be versioned and given
a unique version ID when they are modified by future requests. Note the following:
...
* Objects that are stored in your bucket before you set the versioning state
  ...
  objects in future requests. For more information, see [Working with objects in a versioning-enabled bucket](./manage-objects-versioned-bucket.html).
* The bucket owner (or any user with appropriate permissions) can suspend
  ...
  objects in future requests. For more information, see [Working with objects in a versioning-suspended bucket](./VersionSuspendedBehavior.html).

## Using S3 Versioning with S3 Lifecycle

To customize your data retention approach and control storage costs, use object
versioning with S3 Lifecycle. For more information, see [Managing the lifecycle of objects](./object-lifecycle-mgmt.html). For
information about creating S3 Lifecycle configurations using the AWS Management Console, AWS CLI, AWS SDKs, or
the REST API, see [Setting an S3 Lifecycle configuration on a bucket](./how-to-set-lifecycle-configuration-intro.html).
...
noncurrent, object versions.) For more information, see [Setting an S3 Lifecycle configuration on a bucket](./how-to-set-lifecycle-configuration-intro.html).

For information about working with S3 Versioning, see the following topics.

###### Topics

* [How S3 Versioning works](./versioning-workflows.html)
* [Enabling versioning on buckets](./manage-versioning-examples.html)
* [Configuring MFA delete](./MultiFactorAuthenticationDelete.html)
* [Working with objects in a versioning-enabled bucket](./manage-objects-versioned-bucket.html)
* [Working with objects in a versioning-suspended bucket](./VersionSuspendedBehavior.html)
* [Troubleshooting versioning](./troubleshooting-versioning.html)
